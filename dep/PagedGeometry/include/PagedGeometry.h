...
public:
	...
	*/
	...
	The PagedGeometry engine comes pre-installed with several page types: EntityPage,
...
The PagedGeometry engine comes pre-installed with several GeometryPage sub-classes
(EntityPage, BatchPage, and ImpostorPage). These "page types" can all be supplied to
...
virtual void addEntity(Entity *ent, const Vector3 &position, const Quaternion &rotation, const Vector3 &scale) = 0;
virtual void build() {}
...
\see The BatchPage or EntityPage code for examples of how page types are implemented (the
...
public:
	...
	*/
	...
	is called.
	*/
	virtual void addEntity(Ogre::Entity *ent, const Ogre::Vector3 &position, const Ogre::Quaternion &rotation, const Ogre::Vector3 &scale) = 0;
...
page loader class's loadPage() funtion. It's completely up to you how this function
...
public:
	...
	*/
	...
	\brief Destructor
	This is defined here so the destructors of derived classes are called properly. Whether
	or not you actually impliment a destructor is up to you.
	*/
	virtual ~PageLoader() {}
...
protected:
	...
	*/
	...
	{
		geomPage->addEntity(ent, position, rotation, scale);
...
template <class PageType> inline GeometryPageManager* PagedGeometry::addDetailLevel(Ogre::Real maxRange, Ogre::Real transitionLength)
{
	...
	Ogre::Root *root;
	if (!root->getSingleton().getRenderSystem()->getCapabilities()->hasCapability(Ogre::RSC_VERTEX_PROGRAM))
		transitionLength = 0;
...
template <class PageType> inline void GeometryPageManager::initPages(const TBounds& bounds)
{
	...
	else
	{
		...
		// 4 is extra padding for cashing
		geomGridX = (gridBounds.width() / mainGeom->getPageSize()) + 4;
