...
/*! \brief  Smart pointer with a scoped locking mechanism (recursive).
 *
 * This class is a wrapper for a volatile pointer. It enables synchronized access to the
 * internal pointer by locking the passed recursive_mutex.
 */
  template <typename T>
  class recursive_locking_ptr 
  : private noncopyable
  {
    T* m_obj;                     //!< The instance pointer. 
    recursive_mutex & m_mutex;     //!< Mutex is used for scoped locking.

  public:
    /// Constructor.
    recursive_locking_ptr(volatile T& obj, const volatile recursive_mutex& mtx)
      : m_obj(const_cast<T*>(&obj))
      , m_mutex(*const_cast<recursive_mutex*>(&mtx))
    {   
      // Lock mutex
      boost::detail::thread::lock_ops<recursive_mutex>::lock(m_mutex);   
    }


    /// Destructor.
    ~recursive_locking_ptr()
    { 
      // Unlock mutex
      boost::detail::thread::lock_ops<recursive_mutex>::unlock(m_mutex);
    }
